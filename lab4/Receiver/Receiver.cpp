...
int main() {
	...
	cout << "Write the name of the binary file" << endl;
	string file_name;
	cin >> file_name;

	cout << "Enter the maximum number of entries in the file" << endl;
	int n_records;
	cin >> n_records;
	...
	char input;
	cout << "Enter any symbol to read messages from the file (0 - termination of the stream)" << endl;
	while (true) {
		cin >> input;
		if (input == '0')
			break;

		cout << "Accepted " << rec.pop() << endl;
	}
